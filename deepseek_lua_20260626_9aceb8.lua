--[[
    Script Hack Roblox - Muigl Reborn Final (3 ngón giữ 1 giây)
    Key: khanhhuy
    Giao diện nâng cấp, menu rộng, tab ngang.
    Kích hoạt: chạm 3 ngón và giữ 1 giây để mở/tắt menu (có animation trượt).
    ESP chỉ hiện kẻ địch (team check luôn bật), box to, tracer, tên, máu, khoảng cách.
    Hiển thị số kẻ địch ở giữa trên màn hình khi bật ESP.
...
local SETTINGS = {
    ...
    Smoothness = 0.15,
    FOV = 200,
    TeamCheck = true,   -- luôn bật, không có tuỳ chọn tắt
    VisibleCheck = true,
    Speed = false
}
...
function loadMain()
    ...
    -- Nhãn hiển thị số kẻ địch (chỉ khi ESP bật)
    local EnemyCountLabel = Instance.new("TextLabel", MainGui)
    EnemyCountLabel.Size = UDim2.new(0,200,0,30)
    EnemyCountLabel.Position = UDim2.new(0.5,-100,0,60)  -- gần giữa trên
    ...
    addToggle(visTab, "FOV Circle", true, function(v) end) -- ảnh hưởng khi aimbot bật
