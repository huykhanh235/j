--[[
    Script Hack Roblox - Muigl Final Fix (Hiển thị menu đầy đủ, vuốt 3 ngón lên mở)
    Key: khanhhuy
    Menu: giao diện rõ ràng, tab ngang, toggle và slider hiển thị đúng.
...
function loadMain()
    ...
    Menu.Size = UDim2.new(0,360,0,420) -- tăng chiều cao để chứa các control
    Menu.Position = UDim2.new(0.5,-180,1,0)
    ...
    ContentContainer.ZIndex = 220
    Instance.new("UICorner", ContentContainer).CornerRadius = UDim.new(0,12)

    -- Tạo các tab và thêm UIListLayout cho từng tab để tự động sắp xếp
    ...
    for _, name in ipairs(Tabs) do
        ...
        frame.Visible = false
        frame.ZIndex = 230
        -- Thêm UIListLayout để tự động xếp các control theo chiều dọc
        ...
        -- Thêm UIPadding để có khoảng cách với viền
    ...
    -- Hàm tạo Toggle (sử dụng UIListLayout, không cần đặt Position)
    local function addToggle(parent, name, default, callback)
        local frame = Instance.new("Frame")
        frame.Size = UDim2.new(1,0,0,36) -- Chiều rộng full, chiều cao cố định
    ...
    fovInput.Size = UDim2.new(1,-16,0,20)
    fovInput.Position = UDim2.new(0,8,0,20)
    fovInput.Text = "200"
    ...
    smoothFrame.BackgroundTransparency = 0.5
    Instance.new("UICorner", smoothFrame).CornerRadius = UDim.new(0,6)
    ...
    smoothInput.Size = UDim2.new(1,-16,0,20)
    smoothInput.Position = UDim2.new(0,8,0,20)
    smoothInput.Text = "0.15"
    smoothInput.BackgroundColor3 = Color3.fromRGB(70,70,70)
    ...
    addToggle(visTab, "FOV Circle", true, function(v) end) -- chỉ bật/tắt vòng tròn FOV (vẽ luôn)

    -- Tab Other
    local otherTab = ContentFrames["Other"]
    addToggle(otherTab, "Speed Hack", false, function(v) SETTINGS.Speed = v end)

    -- ===== KÍCH HOẠT MENU (VUỐT 3 NGÓN LÊN) =====
    ...
    local function toggleMenu()
        ...
        if menuOpen then
            Menu.Visible = true
            Menu.Position = UDim2.new(0.5,-180,1,0)
            TweenService:Create(Menu, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = UDim2.new(0.5,-180,0.5,-210)}):Play()
        else
            TweenService:Create(Menu, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Position = UDim2.new(0.5,-180,1,0)}):Play()
            wait(0.35)
            Menu.Visible = false
        end
    end
