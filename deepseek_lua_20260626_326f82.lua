--[[
    Script Hack Roblox - Muigl Reborn Final
    Key: khanhhuy
    Giao diện nâng cấp, menu rộng, tab ngang.
    Kích hoạt menu: chạm 3 ngón 3 lần (không có nút nổi).
    ESP: box to, tracer, tên, máu, khoảng cách. Mặc định chỉ hiện kẻ địch (team check).
    Aimbot: silent aim, FOV circle, chỉ bắn kẻ địch.
    Luôn hiện số lượng người chơi trên màn hình.
    Đã loại bỏ Fly.
    palofsc
--]]
...
local SETTINGS = {
    ...
    Smoothness = 0.15,
    FOV = 200,
    TeamCheck = true,  -- luôn bật kiểm tra đội
    VisibleCheck = true,
    Speed = false,
    -- Fly đã bị xoá
}

-- ===== KEY SYSTEM (GIAO DIỆN ĐẸP) =====
...
function loadMain()
    ...
    -- Nhãn hiển thị số lượng người chơi (luôn hiện)
    local PlayerCountOverlay = Instance.new("TextLabel", MainGui)
    PlayerCountOverlay.Size = UDim2.new(0,150,0,25)
    PlayerCountOverlay.Position = UDim2.new(0,10,0,10)
    PlayerCountOverlay.BackgroundTransparency = 0.6
    PlayerCountOverlay.BackgroundColor3 = Color3.fromRGB(0,0,0)
    PlayerCountOverlay.TextColor3 = Color3.fromRGB(255,255,255)
    PlayerCountOverlay.Font = Enum.Font.GothamBold
    PlayerCountOverlay.TextSize = 14
    PlayerCountOverlay.Text = "Người chơi: 0"
    PlayerCountOverlay.ZIndex = 999
    Instance.new("UICorner", PlayerCountOverlay).CornerRadius = UDim.new(0,8)

    -- Cập nhật số người chơi mỗi giây
    spawn(function()
        while wait(1) do
            PlayerCountOverlay.Text = "Người chơi: "..#Players:GetPlayers()
        end
    end)

    -- Menu chính (không có nút nổi, mở bằng chạm 3 ngón 3 lần)
    ...
    -- Widget: Toggle (tăng độ tương phản)
    ...
    -- Không có toggle Team Check vì đã bật mặc định
    ...
    smoothFrame.BackgroundTransparency = 0.9
    smoothFrame.BackgroundColor3 = Color3.fromRGB(30,30,30)
    ...
    -- Tab Visuals (FOV Circle)
    local visTab = ContentFrames["Visuals"]
    addToggle(visTab, "Hiển thị FOV Circle", true, function(v) end) -- placeholder

    -- Tab Other (chỉ còn Speed Hack)
    local otherTab = ContentFrames["Other"]
    addToggle(otherTab, "Speed Hack", false, function(v) SETTINGS.Speed = v end)

    -- ===== ĐIỀU KHIỂN MENU (3 ngón 3 lần) =====
    local menuOpen = false
    local tapCount = 0
    local lastTapTime = 0
    local requiredTaps = 3
    ...
    local function toggleMenu()
        ...
        if menuOpen then
            Menu.Visible = true
            Menu.Position = UDim2.new(0.5,-270,1,0)
            TweenService:Create(Menu, TweenInfo.new(0.35, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = UDim2.new(0.5,-270,0.5,-220)}):Play()
        else
    ...
    UserInputService.TouchEnded:Connect(function(touch, processed)
        ...
        if count == 0 and (tick() - lastTapTime < tapTimeout) then
            -- Nhấc hết ngón tay sau khi từng chạm 3 ngón
            tapCount = tapCount + 1
            lastTapTime = tick()
            if tapCount >= requiredTaps then
                tapCount = 0
                toggleMenu()
            end
        elseif count > 0 then
            -- Reset nếu không đủ 0 (người dùng vẫn giữ ngón)
        end
    end)

    -- Reset tapCount nếu quá thời gian
    spawn(function()
        while wait(0.5) do
            if tick() - lastTapTime > tapTimeout and tapCount > 0 then
                tapCount = 0
            end
        end
    end)
    ...
    local function updateESP()
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            ...
            -- Box to hơn (gấp 1.8 lần chiều cao đầu -> chân)
            local rawH = math.abs(headPos.Y - rootPos.Y)
            local boxH = rawH * 1.8
            local boxW = boxH * 0.65
            local boxX = headPos.X - boxW/2
            local boxY = headPos.Y - boxH * 0.2  -- dịch lên một chút để bao phủ đầu
    ...
    print("Muigl Reborn đã sẵn sàng. Chạm 3 ngón 3 lần để mở menu.")
end
