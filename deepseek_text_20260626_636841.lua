--[[
    ...
    Key: khanhhuy
    Mở menu: giữ 3 ngón tay trên màn hình 1 giây (không nhấc lên).
...
function loadMain()
    ...
    -- ===== KÍCH HOẠT MENU (ĐÃ FIX LỖI VUỐT) =====
    local menuOpen = false
    local touchStartTimes = {}
    local touchStartPositions = {}
    local touchMovedDistance = {}
    local holdTimer = 0
    local threeActive = false
    ...
    UserInputService.TouchStarted:Connect(function(touch, processed)
        ...
        touchMovedDistance[touch] = 0
        local count = 0
        for _ in pairs(touchStartTimes) do count = count + 1 end
        if count == 3 and not threeActive then
            threeActive = true
            holdTimer = tick()
        end
    end)
    ...
    UserInputService.TouchMoved:Connect(function(touch, processed)
        if processed then return end
        if touchStartPositions[touch] then
            local currentPos = Vector2.new(touch.Position.X, touch.Position.Y)
            local delta = (currentPos - touchStartPositions[touch]).Magnitude
            touchMovedDistance[touch] = (touchMovedDistance[touch] or 0) + delta
            touchStartPositions[touch] = currentPos
        end
    end)

    UserInputService.TouchEnded:Connect(function(touch, processed)
        if processed then return end
        if touchStartTimes[touch] then
            touchStartTimes[touch] = nil
        end
        touchStartPositions[touch] = nil
        touchMovedDistance[touch] = nil
        local count = 0
        for _ in pairs(touchStartTimes) do count = count + 1 end
        if count < 3 then
            threeActive = false
    ...
    spawn(function()
        while wait(0.1) do
            if threeActive and holdTimer > 0 then
                if tick() - holdTimer >= 1 then
                    local totalDist = 0
                    for _, dist in pairs(touchMovedDistance) do
                        totalDist = totalDist + (dist or 0)
                    end
