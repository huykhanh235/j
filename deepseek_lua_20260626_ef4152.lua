--[[
    Script Hack Roblox - Muigl Reborn Final (Không dùng Drawing, chỉ dùng GUI)
    Key: khanhhuy
    Yêu cầu: chạm 3 ngón giữ 1 giây để mở/tắt menu (animation trượt).
    ESP: Box, Fill Box, Tracer, Tên, Máu, Khoảng cách. Số lượng kẻ địch hiện phía trên.
    Aimbot: Silent Aim, FOV Circle, Team Check (luôn bật), Visible Check.
    Giao diện rộng, tab ngang, chữ rõ.
    palofsc
--]]
...
function loadMain()
    ...
    -- Số lượng kẻ địch (hiện khi ESP bật)
    ...
    -- Widget: Toggle (độ tương phản cao)
    ...
    smoothFrame.BackgroundTransparency = 0.4
    ...
    -- ===== ĐIỀU KHIỂN MENU (giữ 3 ngón 1 giây) =====
    local menuOpen = false
    local holdTimer = 0
    local threeFingersHeld = false
    local activeTouches = {}
    ...
    spawn(function()
        while wait(0.1) do
            if threeFingersHeld and holdTimer > 0 then
                if tick() - holdTimer >= 1 then
                    threeFingersHeld = false
                    holdTimer = 0
                    toggleMenu()
                end
    ...
    local function createESPElement(className, properties)
        local element = Instance.new(className)
        for prop, value in pairs(properties) do
            element[prop] = value
        end
        element.Parent = ESPGui
        table.insert(espElements, element)
        return element
    end
    ...
    local function updateESP()
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            ...
            -- Tracer (đường thẳng từ tâm dưới đến chân)
            if SETTINGS.ESP_Tracer then
                local tracer = createESPElement("Frame", {
                    Name = "Tracer",
                    Size = UDim2.new(0, 1, 0, (rootPos.Y - Camera.ViewportSize.Y)),
                    Position = UDim2.new(0, headPos.X, 0, Camera.ViewportSize.Y),
                    BackgroundColor3 = Color3.fromRGB(255,255,255),
                    BackgroundTransparency = 0.7,
                    BorderSizePixel = 0,
                    Rotation = math.deg(math.atan2(rootPos.Y - Camera.ViewportSize.Y, headPos.X - Camera.ViewportSize.X/2))
                })
            ...
            if SETTINGS.ESP_Health then
                ...
                createESPElement("Frame", {
                    ...
                    Position = UDim2.new(0, barX, 0, barY),
                    BackgroundColor3 = Color3.fromRGB(20,20,20),
                    BorderSizePixel = 0
                })
                createESPElement("Frame", {
                    Name = "HealthFill",
                    Size = UDim2.new(0, barWidth, 0, barHeight * hpPct),
                    Position = UDim2.new(0, barX, 0, barY + barHeight * (1 - hpPct)),
    ...
    local function drawFOV()
        for _, frame in ipairs(fovFrames) do frame:Destroy() end
        fovFrames = {}
    ...
    print("Muigl Reborn sẵn sàng. Giữ 3 ngón 1 giây để mở menu.")
end
